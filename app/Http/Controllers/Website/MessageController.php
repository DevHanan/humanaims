...
class MessagesController extends Controller
{
    ...
    public function index($id = null)
    {
        ...
        // prepare id
        return view('website.messages.pages.app', [
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('members',  function ($join) {
            $join->on('messages.from_id', '=', 'members.id')
                ->orOn('messages.to_id', '=', 'members.id');
        })
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $user) {
                if ($user->id != Auth::id()) {
                    // Get user data
                    $userCollection = Member::where('id', $user->id)->first();
    ...
    {
        // Get user data
        $userCollection = Member::where('id', $request['user_id'])->first();
    ...
    public function search(Request $request)
    {
        ...
        $records = Member::where('name', 'LIKE', "%{$input}%");
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? Member::where('id', Auth::user()->id)->update(['dark_mode' => 1])  // Make Dark
                : Member::where('id', Auth::user()->id)->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    $update = Member::where('id', Auth::user()->id)->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        $update = $request['status'] > 0
            ? Member::where('id', $request['user_id'])->update(['active_status' => 1])
            : Member::where('id', $request['user_id'])->update(['active_status' => 0]);
        // send the response
        return Response::json([
            'status' => $update,
        ], 200);
